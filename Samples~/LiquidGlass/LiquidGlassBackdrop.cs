...
{
    /// <summary>
    /// The 3D scene behind the Liquid Glass sample: a full-screen animated aurora wallpaper with
    /// contour lines, and glossy orbs drifting in front of it. Liquid glass is only as interesting
    ...
    ///
    /// <see cref="Exposure"/>, <see cref="Warmth"/> and <see cref="Calm"/> are driven by the UI
    /// (brightness level, Night Shift, Focus) and eased here so the scene responds smoothly.
    /// </summary>
    [AddComponentMenu("Quill/Samples/Liquid Glass Backdrop")]
    public sealed class LiquidGlassBackdrop : MonoBehaviour
    {
        ...
        private static readonly int IdExposure = Shader.PropertyToID("_LGExposure");
        private static readonly int IdWarmth = Shader.PropertyToID("_LGWarmth");
        private static readonly int IdCalm = Shader.PropertyToID("_LGCalm");
        ...
        private void Start()
        {
            if (WallpaperShader == null) WallpaperShader = Shader.Find("Quill/Samples/LiquidGlassWallpaper");
            if (OrbShader == null) OrbShader = Shader.Find("Quill/Samples/LiquidGlassOrb");

            _exposure = Exposure; _warmth = Warmth; _calm = Calm;
            PushGlobals();

            if (WallpaperShader != null) BuildWallpaper();
            else Debug.LogWarning("[Quill] Liquid Glass sample: wallpaper shader missing.");

            if (OrbShader != null) BuildOrbs();
            else Debug.LogWarning("[Quill] Liquid Glass sample: orb shader missing.");
        }
        ...
        private void Update()
        {
            ...
            // Focus mode also slows the scene down.
        ...
        private void BuildWallpaper()
        {
            ...
            var mesh = new Mesh { name = "Liquid Glass Wallpaper" };
            ...
            var mat = new Material(WallpaperShader) { name = "Liquid Glass Wallpaper" };
        ...
        {
            var mat = new Material(OrbShader) { name = "Liquid Glass Orb" };
            _owned.Add(mat);
            ...
            // always something passing behind both the left cards and the right control grid.
            Vector3[] homes =
            {
